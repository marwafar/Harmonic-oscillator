...
bleach=np.genfromtxt("response_GB_R.txt", delimiter=" ")
emission=np.genfromtxt("response_SE_R.txt", delimiter=" ")
absorp=np.genfromtxt("response_ESA_R.txt", delimiter=" ")
...
spect=open("spectra_rephase_T0-zp.txt","w+")
for i in range(n_step):
 for j in range(n_step):
